...
// Fetch student information with more details
$sql = "SELECT s.student_id, s.lrn, s.year_level_id, yl.level_name, yl.education_stage,
...
// Fetch available grade levels for this student
$grade_levels_sql = "SELECT DISTINCT yl.year_level_id, yl.level_name, yl.education_stage
                     FROM tbl_grades g
...
if ($selected_grade_level > 0) {
    ...
    $stmt->bind_param("ii", $student_id, $selected_grade_level);
    $stmt->execute();
    $result = $stmt->get_result();
...
// Fetch detailed grade computation for a specific subject (if requested)
$subject_details = null;
if (isset($_GET['subject_id']) && $selected_grade_level > 0) {
    $subject_id = intval($_GET['subject_id']);
    
    // Get subject name
    $subject_sql = "SELECT subject_name FROM tbl_subjects WHERE subject_id = ?";
    ...
    // Get detailed scores from teacher's scoring system
    $detailed_scores_sql = "
        SELECT quiz1, quiz2, midterm, activity1, activity2, project, assignments, final, 
               created_at, updated_at
        FROM tbl_scores 
        WHERE student_id = ? AND subject_id = ?
    ";
    $stmt = $conn->prepare($detailed_scores_sql);
    $stmt->bind_param("ii", $student_id, $subject_id);
    $stmt->execute();
    $detailed_result = $stmt->get_result();
    $subject_details = $detailed_result->fetch_assoc();
    
    if ($subject_details) {
        $subject_details['subject_name'] = $subject_name;
        
        // Calculate component averages
        $quizzes = array_filter([$subject_details['quiz1'], $subject_details['quiz2']], function($v) {
            return $v !== null && $v > 0;
        });
        $subject_details['quiz_avg'] = !empty($quizzes) ? array_sum($quizzes) / count($quizzes) : null;
        
        $activities = array_filter([$subject_details['activity1'], $subject_details['activity2']], function($v) {
            return $v !== null && $v > 0;
        });
        $subject_details['activity_avg'] = !empty($activities) ? array_sum($activities) / count($activities) : null;
        
        // Calculate overall average
        $all_scores = array_filter([
            $subject_details['quiz1'], $subject_details['quiz2'], 
            $subject_details['midterm'], $subject_details['activity1'], 
            $subject_details['activity2'], $subject_details['project'], 
            $subject_details['assignments'], $subject_details['final']
        ], function($v) {
            return $v !== null && $v > 0;
        });
        
        $subject_details['overall_avg'] = !empty($all_scores) ? array_sum($all_scores) / count($all_scores) : null;
    }
}
?>
...
<head>
    ...
    <title>EduTrack - Student Records</title>
    ...
    <style>
        ...
        @media print {
            .level-selector, .semester-filter, .print-btn, .sidebar, .header { display: none; }
...
<body>
    <div class="container-fluid">
        <div class="row">
            <!-- Sidebar - Consistent with student activities page -->
            <div class="col-md-3 col-lg-2 sidebar">
                ...
                <ul class="sidebar-menu">
                    ...
                    <li><a href="student_records.php" class="active"><i class="fas fa-chart-line"></i> Records</a></li>
            ...
            <div class="col-md-9 col-lg-10 ml-sm-auto px-4">
                <div class="header">
                    <h3><i class="fas fa-file-alt me-2"></i> Student Records</h3>
                    <div class="user-info">
                ...
                <div class="main-content">
                    <!-- Welcome Banner - Consistent with student activities page -->
                    <div class="welcome-section">
                        <div class="row align-items-center">
                            <div class="col-md-8">
                                <h1>Welcome, <?= htmlspecialchars(explode(" ", $student_name)[0]) ?>!</h1>
                                <p>View your academic records and grade history</p>
                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="level-selector mb-4 p-3" style="background: #e3f2fd; border-radius: 8px;">
                                ...
                                <?php if (!empty($available_grade_levels)): ?>
                                    <?php foreach ($available_grade_levels as $level): ?>
                                        <a href="student_records.php?grade_level=<?= $level['year_level_id'] ?>" 
                            ...
                            <div class="semester-filter mb-4 p-3" style="background: #f8f9fa; border-radius: 8px;">
                                ...
                                <a href="student_records.php?grade_level=<?= $selected_grade_level ?>&semester=all" 
                                   class="semester-btn <?= $selected_semester == 'all' ? 'active' : '' ?>">
                                    All Semesters
                                </a>
                                <?php foreach ($semesters_available as $semester): ?>
                                    <a href="student_records.php?grade_level=<?= $selected_grade_level ?>&semester=<?= urlencode($semester) ?>" 
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p><strong>Name:</strong> <?= htmlspecialchars(strtoupper($student['last_name'] . ', ' . $student['first_name'])) ?></p>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <?php foreach ($semester_grades as $grade): ?>
                                                            ...
                                                            <tr>
                                                                <td class="subject-name" 
                                                                    onclick="showGradeDetails(<?= $grade['subject_id'] ?>, '<?= htmlspecialchars($grade['subject_name']) ?>')"
                                                                    title="Click to view grade computation details">
                                                        ...
                                                        <tr style="background: #e0e0e0;">
                                                            ...
                                                            <td class="final-grade">
                                                                <strong>
                                                                    <?= $subjects_count > 0 ? round($total_final_grade / $subjects_count, 2) : '' ?>
                                                                </strong>
                                                            </td>
                            ...
                            <?php else: ?>
                                <div class="empty-state">
                                    ...
                                    <p>Grades will appear here once they are recorded by your teachers.</p>
                                </div>
                            <?php endif; ?>
                            <?php else: ?>
                                <div class="empty-state">
                                    ...
                                    <p>Choose a grade level from the options above to view your academic records.</p>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="gradeDetailsModalLabel">Grade Computation Details</h5>
    ...
    <script>
        function showGradeDetails(subjectId, subjectName) {
            // Show loading state
            ...
            // Set modal title
            document.getElementById('gradeDetailsModalLabel').textContent = 'Grade Details: ' + subjectName;
            ...
            fetch('student_records.php?grade_level=<?= $selected_grade_level ?>&subject_id=' + subjectId)
                .then(response => response.text())
                .then(html => {
                    // This would normally return JSON, but we're handling it in the same file
                    // For now, we'll create the content based on the PHP variable
                    <?php if ($subject_details): ?>
                    document.getElementById('gradeDetailsContent').innerHTML = `
                        <h6 class="mb-3">${subjectName} - Grade Breakdown</h6>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Quiz 1:</span>
                            <span class="grade-value"><?= $subject_details['quiz1'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Quiz 2:</span>
                            <span class="grade-value"><?= $subject_details['quiz2'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Quizzes Average:</span>
                            <span class="grade-value"><?= $subject_details['quiz_avg'] ? round($subject_details['quiz_avg'], 2) : 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Activity 1:</span>
                            <span class="grade-value"><?= $subject_details['activity1'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Activity 2:</span>
                            <span class="grade-value"><?= $subject_details['activity2'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Activities Average:</span>
                            <span class="grade-value"><?= $subject_details['activity_avg'] ? round($subject_details['activity_avg'], 2) : 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Midterm Exam:</span>
                            <span class="grade-value"><?= $subject_details['midterm'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Project:</span>
                            <span class="grade-value"><?= $subject_details['project'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Assignments:</span>
                            <span class="grade-value"><?= $subject_details['assignments'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-detail-item">
                            <span class="grade-component">Final Exam:</span>
                            <span class="grade-value"><?= $subject_details['final'] ?? 'N/A' ?></span>
                        </div>
                        
                        <div class="grade-average mt-3 p-3">
                            <div class="d-flex justify-content-between">
                                <span>Overall Average:</span>
                                <strong><?= $subject_details['overall_avg'] ? round($subject_details['overall_avg'], 2) : 'N/A' ?></strong>
                            </div>
                        </div>
                        
                        <?php if ($subject_details['updated_at']): ?>
                        <div class="text-muted small mt-3">
                            Last updated: <?= date('M j, Y g:i A', strtotime($subject_details['updated_at'])) ?>
                        </div>
                        <?php endif; ?>
                    `;
                    <?php else: ?>
                    document.getElementById('gradeDetailsContent').innerHTML = `
                        <div class="text-center py-4">
                            ...
                            <h6>Detailed grade breakdown not available</h6>
                            <p class="text-muted">The detailed computation for this subject is not currently available.</p>
                        </div>
                    `;
                    <?php endif; ?>
                })
